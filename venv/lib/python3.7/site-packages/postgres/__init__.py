...
class NotASimpleCursor(Exception):
    def __str__(self):
        return (
            "We can only work with subclasses of postgres.cursors."
            "SimpleCursorBase. {} doesn't fit the bill.".format(self.args[0].__name__)
        )
...
class NotAModel(Exception):
    def __str__(self):
        return (
            "Only subclasses of postgres.orm.Model can be registered as "
            "orm models. {} doesn't fit the bill.".format(self.args[0])
        )
...
class NoTypeSpecified(Exception):
    def __str__(self):
        return (
            "You tried to register {} as an orm model, but it has no "
...
class NoSuchType(Exception):
    def __str__(self):
        return (
            "You tried to register an orm model for typname {}, but no "
            "such type exists in the pg_type table of your database.".format(
                self.args[0]
            )
        )
...
class AlreadyRegistered(Exception):
    def __str__(self):
        return "The model {} is already registered for the typname {}.".format(
            self.args[0].__name__, self.args[1]
        )
...
class BadBackAs(Exception):
    def __str__(self):
        return (
            "Bad back_as: {}. Available back_as values are: tuple, "
            "namedtuple, dict, or None (to use the default).".format(self.args[0])
        )
...
class Postgres(object):
    ...
    def __init__(
        self, url, minconn=1, maxconn=10, cursor_factory=SimpleNamedTupleCursor
    ):
        ...
        self.pool = ConnectionPool(
            minconn=minconn, maxconn=maxconn, dsn=dsn, connection_factory=Connection
        )
    ...
    def register_model(self, ModelSubclass, typname=None):
        ...
        n = self.one("SELECT count(*) FROM pg_type WHERE typname=%s", (typname,))
        if n < 1:
